...
- `adminAuth.ts` — `RequestHandler` guarding `/admin/*` via the `X-Admin-Token` header vs `ADMIN_TOKEN`. **Fail-closed:** an unset `ADMIN_TOKEN` makes every admin route return 503 (admin disabled), so an unconfigured server never exposes capture/refresh ops. Timing-safe compare.
...
Routes are defined inline in `src/app.ts` — read it for the full list. `:accountId` parsing is shared via `parseAccountIdParam`; `?range`/`?ranges` parsing via `parseRange`/`parseRanges`. Player profile is `GET /players/:accountId` (WG `account/info`). Admin endpoints: `POST /admin/snapshots/run`, `/admin/vehicles/refresh`, `/admin/wn8/refresh-expected` — all gated by the `X-Admin-Token` header (`lib/adminAuth.ts`, fail-closed on unset `ADMIN_TOKEN`).
