...
Express app (`src/app.ts`) with routes defined inline; shared infrastructure lives in `src/lib/`. Six lib modules today:
...
Defined inline in `src/app.ts`. `GET /health` pings the DB; `GET /players/search` and `GET /players/info` proxy the WG middleman; `GET /players/:accountId/stats?range=` and `GET /players/:accountId/stats/summary?ranges=` compute trailing-window deltas (ranges constrained to 7/14/30). Enrollment into `TrackedAccount` (the work list the daily capture job iterates) happens inside `getPlayerInfo`, gated on WG confirming the account exists — so querying `/players/info` or the stats endpoints auto-enrolls real accounts but never non-existent ones. `POST /admin/snapshots/run` triggers `runCaptureJob()` out of band. All routes send responses via `lib/http.ts` (`sendResult`/`sendApiError`); the `apiErrorHandler` middleware from `lib/middleware.ts` is registered after the routes as the catch-all for thrown errors. `:accountId` parsing is shared via the local `parseAccountIdParam` helper.
